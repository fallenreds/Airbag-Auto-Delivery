...
def build_manager_notes(order: Order, user: Client) -> str:
    ...
    if order.ttn:
        # ТТН живёт здесь, рядом с остальными данными заказа: заметки
        # менеджера — единственное поле карточки, которого касается система.
        # Отсюда же крон забирает номер, вписанный менеджером вручную.
        goods_info += f"\nНомер ТТН: {order.ttn}"
