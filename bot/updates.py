...
from notifications import for_admin, merge_order_notification, new_order_notification, new_order_client_notification, deactivated_notifications, ttn_update_notification, order_in_branch_notifications, payment_doc_uploaded_notification, canceled_notifications, refunded_notifications, cancel_requested_notifications, remonline_created_notification, payment_confirmed_notification, payment_type_changed_notification, cancel_rejected_notification, remonline_sync_failed_notification, delivery_returned_notification, delivery_failed_notification, parcel_destroyed_notification
import asyncio
...
async def _on_delivery_returned(bot, order, record, admin_list):
    await delivery_returned_notification(bot, order, admin_list)


async def _on_delivery_failed(bot, order, record, admin_list):
    await delivery_failed_notification(bot, order, admin_list)


async def _on_parcel_destroyed(bot, order, record, admin_list):
    await parcel_destroyed_notification(bot, order, admin_list)
...
ORDER_EVENT_HANDLERS = {
    ...
    "DELIVERY_RETURNED": _on_delivery_returned,
    "DELIVERY_FAILED": _on_delivery_failed,
    "PARCEL_DESTROYED": _on_parcel_destroyed,
    "MERGED": _on_merged,
