...
# Метка «ТТН» (в любом регистре), после неё до пяти не-цифр (двоеточие, «№»,
# тире, пробелы), затем 10–14 цифр — с пробелами или дефисами внутри, как их
# иногда печатает человек: «2045 1524 7727 76».
TTN_PATTERN = re.compile(r"ТТН[^0-9\n]{0,5}(\d(?:[\s-]?\d){9,13})", re.IGNORECASE)
...
def parse_ttn(notes: str):
    ...
    Менеджер пишет рядом что угодно: «ттн: 2045… (Нова Пошта)», «ТТН №2045…»,
    номер с пробелами, текст до и после. Берём только цифры после метки — от
    десяти до четырнадцати; буквы и знаки в номер не попадают. Старый разбор
    «14 символов после ТТН:» при коротком номере захватывал следующее слово.
    """
    match = TTN_PATTERN.search(notes or "")
    if not match:
        return None
