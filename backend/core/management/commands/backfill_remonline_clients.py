...
class Command(BaseCommand):
    ...
    def handle_stuck_orders(self):
        ...
        Заводим только те, с которыми менеджеру ещё предстоит работать.

        Отменённые пропускаем: карточка, созданная сразу в «Відмова», ничего не
        сообщает, а список закрытых засоряет. Завершённые — тем более: на бою
        таких нашлось одиннадцать штук 2023–2025 годов, импортированных из
        старой системы, и в CRM они выглядели бы как новые заказы трёхлетней
        давности. Черновики (оплата картой до платежа) в CRM и не должны
        попадать.
        """
        stuck = (
            Order.objects.filter(
                remonline_order_id__isnull=True, is_draft=False, is_completed=False
            )
